...
public class NYTimesService implements ReviewServiceInterface{
	
	@WebMethod
	public ConcurrentHashMap<Integer, String[]> readFile() {
	    ConcurrentHashMap<Integer, String[]> nytimesReviews = new ConcurrentHashMap<Integer, String[]>();
		Integer recordNum = 0;
        String line;
        try {
    		BufferedReader br = new BufferedReader(new FileReader("../../NYTimes.txt"));
			while ((line = br.readLine()) != null) {
				recordNum = recordNum++;
				if(line != null && !line.isEmpty()) {
			    	String[] line_split = line.split("\\|");
			    	nytimesReviews.put(recordNum, line_split);
				}
				br.close();
			}
		} catch (IOException e) {
